...
from OptSched import RE_DAG_COST_LOWER_BOUND, RE_DAG_COST, RE_PASS_NUM, getBenchmarkFilePaths, regionSpliter, RE_DAG_INFO
            ...
            if os.path.exists(filePaths[i][bench]):
                ...
                with open(filePaths[i][bench]) as file:
                    ...
                    for block in blocks:
                        ...
                        if dagInfo and dagLwrBound and dagStats:
                            dagName = dagStats.group(1)
                            dagCost = int(dagStats.group(2)) + \
                                int(dagLwrBound.group(1))
                            dagLength = dagStats.group(3)
                            dagIsOptimal = (dagStats.group(4) == 'optimal')
                            dagInstrCount = int(dagInfo.group(2))
                        else:
                            print(block)
                            sys.exit()
...
def main(args):
    ...
    filePaths1 = getBenchmarkFilePaths(args.directory1, args.benchmark)
    filePaths2 = getBenchmarkFilePaths(args.directory2, args.benchmark)
